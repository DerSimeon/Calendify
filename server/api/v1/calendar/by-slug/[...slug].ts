...
export default defineEventHandler(async (event) => {
    ...
    const calendar = await prisma.calendar.findUnique({
        where: { slug },
        include: { CalendarUser: true, CalendarEvent: true }
    });
    ...
    // strip CalendarUser from the response and add "role" field
    delete calendar.CalendarUser;
    calendar.role = calendarUser ? calendarUser.role : 'VIEWER';
