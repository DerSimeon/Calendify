...
async function submit() {
  ...
  saving.value = true
  try {
    await $fetch(`/api/v1/calendars/by-slug/${props.slug}/events`, {
      method: 'POST',
      body: {
        title: title.value,
    ...
    open.value = false
    title.value = ''
    toast.add({ title: 'Event created', description: 'Your appointment was added.' })
    emit('created')
  } catch (e: any) {
    toast.add({ title: 'Could not create event', description: e?.data?.message || e?.message || 'Unknown error', color: 'red' })
  } finally {
    saving.value = false
  }
}

function colorToClass(color: 'violet' | 'fuchsia' | 'emerald' | 'rose' | 'amber') {
  switch (color) {
        ...
        <div class="space-y-4">
          ...
          <div class="grid gap-3 sm:grid-cols-2">
            <UFormGroup label="Start" for="ev-start">
              <UInput id="ev-start" type="datetime-local" v-model="start" :ui="{ base: 'bg-white/5 border-white/10 text-white' }" />
            </UFormGroup>
            <UFormGroup label="End" for="ev-end">
              <UInput id="ev-end" type="datetime-local" v-model="end" :ui="{ base: 'bg-white/5 border-white/10 text-white' }" />
            </UFormGroup>
          </div>
          ...
          <UFormGroup label="Color">
            <div class="flex flex-wrap gap-2">
              <button
                  ...
                  :class="[ c === color ? 'ring-2 ring-white' : '', colorToClass(c).bg, colorToClass(c).border ]"
              />
            </div>
          </UFormGroup>
        </div>
