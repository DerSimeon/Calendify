...
async function handleDelete() {
  const confirmed = confirm('Are you sure you want to delete this calendar? This action cannot be undone.')
  if (!confirmed) return
  await $fetch('/api/v1/calendar', {
    method: 'DELETE',
    body: {id: props.calendar.id},
  })
  props.refresh()
}
</script>
    ...
    <div class="space-y-4">
      ...
      >
        <div class="flex items-center gap-2">
          <UIcon
              :name="props.calendar.visibility === 'PUBLIC' ? 'i-heroicons-globe-alt-16-solid' : 'i-heroicons-lock-closed-16-solid'"
              class="h-4 w-4"
      ...
      <div class="flex items-center gap-2 flex-row">
        ...
        <UButton variant="outline" color="neutral"
                 class=" bg-white/5 border-white/10 text-white font-normal cursor-pointer"
      ...
      <UButton
          variant="outline"
          color="error"
          class="bg-white/5 border-white/10 text-white cursor-pointer"
          @click="handleDelete"
          leading-icon="i-heroicons-trash-16-solid" block label="Delete Calendar"/>
    </div>
  </UCard>
</template>
